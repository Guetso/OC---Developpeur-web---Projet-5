const get = function (url) { // On créé une constante qui est une fonction permettant d'utiliser la méthode GET sur une URL donnée
...
/*
const getItems = function () { // Création d'une fonction "get" qui appliquera la méthode GET pour récupérer l'ensemble des articles
    return new Promise(function (resolve, reject) { // Cette fonction retournera une promesse qui fera :
        const httpRequest = new XMLHttpRequest() // La fonction va créer une requête Http
        httpRequest.onreadystatechange = function () { // On va suivre l'évolution de la requête
            if (httpRequest.readyState === 4) {
        ...
        httpRequest.open("GET", "http://localhost:3000/api/cameras/", true) // Cette requête va utiliser la méthode GET sur l'URL en paramètre, de manière asynchrone
        httpRequest.send() // La fonction envoie la requête au serveur
    })
}
*/
